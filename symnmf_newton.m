...
%SYMNMF Newton-like algorithm for Symmetric NMF (SymNMF)
...
if ~exist('params', 'var')
    H = 2 * sqrt(mean(mean(A)) / k) * rand(n, k);
    maxiter = 10000;
    tol = 1e-4;
    sigma = 0.1;
    beta = 0.1;
